...
abstract class Plugin
{
    ...
    {
        $this->plugins = count($this->plugins) > 0 ? array_merge(
    ...
    protected function identifyAndInstantiateClass(string $name, array $options = []): Plugin
    {
        if (in_array($name, array_keys($this->plugins)) === false) {
            throw new Exceptions(
                "The {$name} plugin wasn\'t found in the list of plugins.", 404
            );
        }
    ...
    {
        if ($returnData) {
            return $this->instance->callebleFunctionActive(
                [$this->object, $method],
                array_values($datas)
            );
        }
        $this->instance->callebleFunctionActive([$this->object, $method],
            array_values($datas));
        return $this;
    }
